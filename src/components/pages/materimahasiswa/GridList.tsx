  ...
  return (
    <ImageList rowHeight={180} cols={cols} sx={{ m: 0 }}>
      {data.map((record) => (
        ...
        >
          {record.berkas != "" && record.berkas != null ? (
            <div className="h-full rounded-xl w-full">
              <PdfPreview
                file={`${process.env.REACT_APP_DATA_PROVIDER}/${record.berkas}`}
              />
              {/* <PDFViewer
                document={{
                  url: `${process.env.REACT_APP_DATA_PROVIDER}/${record.berkas}`,
                }}
              /> */}
            </div>
          ) : (
            <div className="h-full bg-gray-200 rounded-xl w-full place-content-center items-center flex flex-row">
              <Image />
              <div className="text-xs">file Not Found</div>
            </div>
          )}
